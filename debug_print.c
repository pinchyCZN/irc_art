...
int get_key_str(unsigned int key,char *str)
{
	if(key>(sizeof(virtual_keys)/sizeof(char *)))
...
int open_console()
{
	...
	if(0==GetConsoleWindow){
		HMODULE hkern;
		hkern=LoadLibrary(TEXT("kernel32.dll"));
		if(hkern){
			GetConsoleWindow=(HWND (WINAPI *)(void))GetProcAddress(hkern,"GetConsoleWindow");
		}
	}
...
int move_console(int x,int y)
{
	...
	if(GetConsoleWindow){
		...
		hcon=GetConsoleWindow();
		if(hcon){
			result=SetWindowPos(hcon,0,x,y,0,0,SWP_NOZORDER|SWP_NOSIZE);
		}
	}
	return result;
}
