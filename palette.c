	...
	if(init){
		...
		for(x=0;x<pw;x++){
			for(y=0;y<ph;y++){
				...
				if(index>=(sizeof(color_lookup)/4))
					index=0;
...
int paint_current_colors(HWND hwnd,HDC hdc,int id)
{
	BITMAPINFO bmi;
	int c;
	RECT rect;
...
int init_colors()
{
	current_fg=color_lookup[0];
	current_bg=color_lookup[1];
	return 0;
}
int palette_click(int x,int y,int msg)
{
	int index;
	index=get_cindex(x,y,palette_w,palette_h);
	switch(msg){
	case WM_LBUTTONDOWN:
		current_fg=color_lookup[index];
		break;
	case WM_RBUTTONDOWN:
		current_bg=color_lookup[index];
		break;
	}
	return 0;
}
